...
function Main({ navigation }) {
    ...
    const [devs, setDevs] = useState([]); // estado que armazena os devs
    const [techs, setTechs] = useState(''); // estado que armazena as tecnologias da busca
            ...
            <MapView onRegionChangeComplete={handleRegionChanged} initialRegion={currentRegion} style={styles.map} >
                {devs.map(dev => (
                    <Marker key={dev._id} coordinate={{ longitude: dev.location.coordinates[0], latitude: dev.location.coordinates[1] }} >
                        ...
                        <Callout onPress={() => {
                            // navegação - Nome da tela + parâmetros necessários para navegar na página desejada (nome do Profile)
...
// Propriedades para a Estilização
// As propriedades são objetos, ou seja, não se pode utilizar hífens. Por exemplo: background-color passa a ser backgroundColor.
